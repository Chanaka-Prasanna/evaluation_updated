...
# -- Hyperparameters for GRU V2 Model --
MAX_LEN = 500        # max sequence length
EMBED_SIZE = 128
HIDDEN_DIM = 512     # increased from 256 (LSTM) to 512 (GRU)
NUM_LAYERS = 4       # reduced from 6 (LSTM) to 4 (GRU)
...
def encode_sentence(s: str, stoi: dict, max_len: int = MAX_LEN):
    """GRU encoding function matching the original notebook structure"""
...
def predict_regex_to_e_nfa(s: str, model: Seq2SeqGRU, stoi: dict, itos: dict,
    ...
    """GRU prediction function matching the original notebook"""
    model.eval()
