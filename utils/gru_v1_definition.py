...
# -- Hyperparameters for GRU V1 Model --
MAX_LEN = 250        # max sequence length (likely smaller for V1)
EMBED_SIZE = 128
HIDDEN_DIM = 512     
NUM_LAYERS = 4       # detected from model inspection
...
def encode_sentence(s: str, stoi: dict, max_len: int = MAX_LEN):
    """GRU V1 encoding function matching the original notebook structure"""
...
def predict_regex_to_e_nfa(s: str, model: Seq2SeqGRU, stoi: dict, itos: dict,
    ...
    """GRU V1 prediction function matching the original notebook"""
    model.eval()
